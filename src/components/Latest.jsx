

const Latest = () => {
    return (
        <div className="bg-sky-200 pb-24">
            <h1 className="text-xl text-amber-800 text-center font-bold pt-8">Top Requirements for</h1>
            <h1 className="text-5xl text-slate-600 text-center font-bold pt-4 pb-12">Job success!</h1>

            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 w-11/12 mx-auto gap-4">
            <div className="card w-96 bg-base-100 shadow-xl">
  <figure><img src="https://i.ibb.co/tDnctLn/pexels-yan-krukau-7793691.jpg" alt="" /></figure>
  <div className="card-body">
    <h2 className="card-title">
      Web Development
      <div className="badge badge-secondary"></div>
    </h2>
    <p>HTML/CSS skills. If you hope to become a web developer, you must understand HTML and CSS. ...
     Analytical skills. ...
     Responsive design skills. ...
    JavaScript skills. ...
    Interpersonal skills.
      Testing and debugging skills. ...</p>
    
  </div>
</div>

<div className="card w-96 bg-base-100 shadow-xl">
  <figure><img src="https://i.ibb.co/tDnctLn/pexels-yan-krukau-7793691.jpg" alt="" /></figure>
  <div className="card-body">
    <h2 className="card-title">
      Digital Marketing
      <div className="badge badge-secondary"></div>
    </h2>
    <p>Video Marketing,
SEO & SEM,
Content Marketing,
Data & Analytics,
Design Thinking & Planning,
Social Media,
Email,
Be Tech Savvy,
Persuasion,
The Unusual!</p>
    
  </div>
</div>



<div className="card w-96 bg-base-100 shadow-xl">
  <figure><img src="https://i.ibb.co/tDnctLn/pexels-yan-krukau-7793691.jpg" alt="" /></figure>
  <div className="card-body">
    <h2 className="card-title">
    Graphics Design
      <div className="badge badge-secondary"></div>
    </h2>
    <p> 1. Creativity · 2. Communication · 3. Typography · 4. Adobe's Creative Apps · 5. Interactive Media · 6. Coding · 7. Branding ·. ...</p>
    
  </div>
</div>





            </div>
            
        </div>
    );
};

export default Latest;